...
router.route('/api/v1/scene_backgrounds')
    /**
     * @openapi
     * '/api/v1/scene_backgrounds':
     *  get:
     *     tags:
     *       - Scene Background Controller
     *     summary: Fetch all scene backgrounds
     *     security:
     *      - bearerAuth: []
     *     parameters:
     *      - in: query
     ...
     */
    .get(Middleware.AuthorizeJWT, Middleware.AuthorizePermissionJWT("scene-backgrounds:index"), async (req, res) => {
        try {
            ...
            res.send({ 
                ...
                count, 
                pages,
                ...LinkService.paginateLinks(`api/v1/scene_backgrounds`, parseInt(page), pages),
            })
        } catch (error) {
...
router.route('/api/v1/scene_background/:client_side_uuid')
    /**
     * @openapi
     * '/api/v1/scene_background/{client_side_uuid}':
     *  get:
     *     tags:
     *       - Scene Background Controller
     *     summary: Fetch a single scene background
     *     security:
     *      - bearerAuth: []
     *     parameters:
     *      - in: path
     ...
     *        description: Internal Server Error
     */
    .get(Middleware.AuthorizeJWT, Middleware.AuthorizePermissionJWT("scene-backgrounds:show"), async (req, res) => {
        try {
            const { client_side_uuid } = req.params
            const response = await queryService.invoke(new ReadOneQuery(client_side_uuid))
            res.send({
                ...response,
                ...LinkService.entityLinks(`api/v1/scene_background/${client_side_uuid}`, "GET", [
    ...
    *     tags:
    *       - Scene Background Controller
    *     summary: Update a scene background
    *     security:
    *      - bearerAuth: []
    *     parameters:
    *     - in: path
    ...
    *        description: Internal Server Error
    */
    .patch(Middleware.AuthorizeJWT, Middleware.AuthorizePermissionJWT("scene-backgrounds:put"), async (req, res) => {
        try {
            const { client_side_uuid } = req.params
            const { hex } = req.body
